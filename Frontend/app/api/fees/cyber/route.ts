...
export async function GET(request: NextRequest) {
  ...
  const rows = await db.collection("core_cyberfeeentry").find(filter).sort({ created_at: -1 }).toArray();
...
export async function POST(request: NextRequest) {
  ...
  const db = await getDb();
  const last = await db.collection("core_cyberfeeentry").find({}).sort({ id: -1 }).limit(1).toArray();
  const doc = {
  ...
  await db.collection("core_cyberfeeentry").insertOne(doc);
